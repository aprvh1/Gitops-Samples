# Secret Expressions Directly in Kubernetes Manifest

This example demonstrates using Harness Secret Expressions **directly in a Kubernetes Secret manifest** - the most straightforward approach.
...
```
secret-expressions-in-manifest/
├── README.md           (this file)
├── secret.yaml         ⭐ Secret with Harness expressions - START HERE
├── deployment.yaml     Sample deployment using the secrets
└── service.yaml        Sample service exposing the app
```
...
```bash
git add secret-expressions-in-manifest/
...
3. Configure:
   ...
   - **Path**: `GitOps-Secret-Expressions/secret-expressions-in-manifest`
...
# Or using kubectl:
kubectl delete -f secret-expressions-in-manifest/ -n default
```
